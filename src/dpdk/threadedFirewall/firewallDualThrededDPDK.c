...
lcore_main(void *args) {
    uint16_t recv_port = (uint16_t) args;
    uint16_t snd_port = (uint16_t) ((recv_port) ^ 1);
    int i;
    ...
    *bufs = calloc(BURST_SIZE, sizeof(struct rte_mbuf));
    ...
    while (1) {
        ...
        // Get burst of RX packets, from first port of pair.
            ...
            const uint16_t nb_rx = receive(recv_port, bufs);


            if (likely(nb_rx > 0)) {
                for (i = 0; i < nb_rx; i = i + 1) {
                    decideAndForward(snd_port, bufs[i]);
#if DEBUGGING > 0
            sprintf(tmp, "\n---------------------------------------------------\n");
            strcat(debug, tmp);
            printf("%s\n", debug);
...
main(int argc, char *argv[]) {
    ...
    for (port = 0; port < 2; port++) {
        ...
        if(rte_eal_remote_launch(lcore_main, port, lcore_id)<0){
            printf("failed to init Thread\n");
        }
        //if(rte_ctrl_thread_create(&ports[port], "directed forwarder\0", NULL, lcore_main, &port)<0){
        //    printf("failed to init Thread\n");
        //}
