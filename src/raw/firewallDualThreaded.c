...
#include <unistd.h> //lazy wait (sleep)
...
void decideAndForward(const int *socket, uint8_t *buf, const int *numbytes) {
    int allowed, id;
...
#if DEBUGGING > 3
        if(allowed)
            sprintf(tmp, "Allowed on thread %i\n", threadnum);
        else
            sprintf(tmp, "Blocked on thread %i\n", threadnum);
        strcat(debug[threadnum], tmp);
    ...
    printf("%s\n", debug[threadnum]);
    fflush(stdout);
    memset(debug[threadnum],0,strlen(debug[threadnum]));
#endif
}


void *directedConnection(void *arg) {
    int rifidx, wifidx, run;
    run = 1;
...
#if DEBUGGING > 0
    threadnum = rifidx;
    sprintf(origin, "%s", "child");
#endif
    while (run) {
        numbytes = receive(&sockets[rifidx], buf);
        if (numbytes < 0) {
            printf("Error reading data");
            run = 0;
        } else if (numbytes > 0) {
            decideAndForward(&sockets[wifidx], buf, &numbytes);
        }
    }
}
...
#if DEBUGGING > 0
    memset(debug[0],0,strlen(debug[0]));
    memset(debug[1],0,strlen(debug[1]));
#endif
    arg1 = 0;
    arg2 = 1;

    if (pthread_create(&thr1, NULL, directedConnection, &arg1) != 0) {
        printf("Error pthread_create");
        return -1;
    }
    CPU_ZERO(&set1);
    CPU_SET(0, &set1);
    if (pthread_setaffinity_np(thr1, sizeof(cpu_set_t), &set1) != 0) {
        printf("Error set thread affinity");
        return -1;
    }

    if (pthread_create(&thr2, NULL, directedConnection, &arg2) != 0) {
        printf("Error pthread_create");
        return -1;
    }
    CPU_ZERO(&set2);
    CPU_SET(2, &set2);
    if (pthread_setaffinity_np(thr2, sizeof(cpu_set_t), &set2) != 0) {
        printf("Error set thread affinity");
        return -1;
    }
