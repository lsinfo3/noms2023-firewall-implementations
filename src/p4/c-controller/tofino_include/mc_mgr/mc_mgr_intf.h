 ...
 * 4 (A,B,C and D) ASICs with 2, 3, 6 and 8 members respectivly.  ASIC C would
 * have a left count of 2+3=5 and a right count of 8.  When selecting a member
 * it would use index = hash % (5 + 6 + 8).  If the index is 0-7, it is assumed
 * ASIC D will make a copy so ASIC C will not.  If the index is 8-13 ASIC C will
 * select local LAG member 0-5.  If the index is more than 13 it is assumed
 * either ASIC A or B will make a copy so ASIC C will not.
 * @param shdl Session handle.
 * @param dev The ASIC id.
 * @param lag_id The LAG id.
...
/* Needed for UT only. */
void mc_mgr_tree_size(int dev, int pipe, int mgid, int *l1_cnt, int *l2_cnt);
bf_mc_node_hdl_t mc_mgr_tree_first_handle(int dev, int pipe, int mgid);
int mc_mgr_rdm_alloc_cnt(int dev, int size);
uint32_t mc_mgr_l1_hdl_to_rdm_addr(int dev,
                                   int pipe,
                                   bf_mc_node_hdl_t node_hdl);
bool mc_mgr_get_port72_last(int dev, uint32_t addr);
int mc_mgr_get_port72_pipe(int dev, uint32_t addr);
int mc_mgr_test_misc(int dev);
