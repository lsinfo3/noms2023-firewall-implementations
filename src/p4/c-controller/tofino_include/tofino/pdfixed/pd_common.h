...
// enums share namspace in C so if I redefine it, I need to change the names...
...
// it is the only place I use int for device id.
// I had to do this so tha the callback could be passed directly to pipe_mgr
// I need to update the whole code to use int instead of uint8_t

typedef void (*p4_pd_idle_tmo_expiry_cb)(int dev_id,
                                         p4_pd_entry_hdl_t entry_hdl,
                                         void *cookie);

typedef void (*p4_pd_idle_time_update_complete_cb)(int dev_id, void *cookie);
...
typedef void (*p4_pd_register_ent_sync_cb)(int device_id, void *cookie);

typedef void (*p4_pd_register_sync_cb)(int device_id, void *cookie);
...
typedef struct p4_pd_lpf_spec {
  ...
  /* A common time constant, in nanoseconds valid only if the flag is not set */
  float time_constant;
...
typedef enum {
  ...
  PD_MIRROR_TYPE_COAL,
  // XXX define I2E, I2I, E2I, E2E separately
  PD_MIRROR_TYPE_MAX
} p4_pd_mirror_type_e;
...
struct p4_pd_pktgen_app_cfg_tof2 {
  p4_pd_pktgen_trigger_type_e trigger_type;
  uint16_t batch_count;
  uint16_t packets_per_batch;
  uint32_t timer_nanosec;
...
typedef enum p4_pd_tbl_dbg_counter_type_e {
  PD_TBL_DBG_CNTR_DISABLED = 0,
  PD_TBL_DBG_CNTR_LOG_TBL_MISS,
  PD_TBL_DBG_CNTR_LOG_TBL_HIT,
  PD_TBL_DBG_CNTR_GW_TBL_MISS,
  PD_TBL_DBG_CNTR_GW_TBL_HIT,
  PD_TBL_DBG_CNTR_GW_TBL_INHIBIT,
  PD_TBL_DBG_CNTR_MAX
...
/**
 * Tbl debug Counter type set for all tables in stage
 * @param dev_tgt Device information.
...
/**
 * Tbl debug Counter get for all tables in stage
 * @param dev_tgt Device information.
 * @param stage_id Stage.
 * @param stage_cntrs Counter values.
 * @return Status of the API call.
 */
...
/**
 * Tbl debug Counter clear for all tables in stage
 * @param dev_tgt Device information.
 * @param stage_id Stage.
 * @return Status of the API call.
 */
 ...
 * @param disable disable timer if true.
 * @return Status of the API call.
 */
 ...
 * @param timer_usec Timer value in usec.
 * @return Status of the API call.
 */
...
/**
 * Check if field name is in scope in the stage
 * @param dev_tgt Device information.
 * @param stage_id Stage.
 * @param dir DIrection.
 * @param field_name Field name.
 * @param field_exists Field exists.
 * @return Status of the API call.
 */
